...
const CONNECT_INSTALL_KEYS_CDN_URL = 'https://connect-install-keys.atlassian.com';
...
function createSessionToken(addon, verifiedClaims, clientKey, settings) {
  ...
  return jwt.encodeSymmetric(baseJwt, settings.sharedSecret);
}

function getKey(keyId) {
  const cdnUrl = `${process.env.CONNECT_KEYS_CDN_URL || CONNECT_INSTALL_KEYS_CDN_URL}/${keyId}`;
...
function decodeSymmetricToken(token, secret, noVerify) {
  return jwt.decodeSymmetric(token, secret, jwt.SymmetricAlgorithm.HS256, noVerify);
}

async function decodeAsymmetricToken(token, noVerify) {
  const publicKey = await getKey(jwt.getKeyId(token));
  return jwt.decodeAsymmetric(token, publicKey, jwt.AsymmetricAlgorithm.RS256, noVerify);
}

async function getVerifiedClaims(addon, req, res, skipQshVerification) {
  return verifyJwtAndGetClaims(addon, req, res, skipQshVerification, false);
}

async function verifyJwtAndGetClaims(addon, req, res, skipQshVerification, isInstallHook) {
  const isSignedInstallHook = isInstallHook && addon.config.signedInstall();
  ...
  try {
    unverifiedClaims = isSignedInstallHook ?
      await decodeAsymmetricToken(token, true) : decodeSymmetricToken(token, "", true); // decode without verification;
  } catch (e) {
    return Promise.reject({
      code: 401,
  ...
  if (!isSignedInstallHook && !settings) {
    return Promise.reject({
      code: 401,
  ...
  let verifiedClaims;
  try {
    verifiedClaims = isSignedInstallHook ? 
      await decodeAsymmetricToken(token, false) : decodeSymmetricToken(token, secret, false);
  } catch (error) {
    return Promise.reject({
      code: 400,
  ...
  // Skip configuring session token for signed install hook
  let sessionToken = null;
  if (!isSignedInstallHook) {
  ...
  const verifiedParams = {
    clientKey,
    hostBaseUrl: settings ? settings.baseUrl : '',
    token: sessionToken,
    key: (settings && settings.key) || addon.key,
  };
...
function verifyInstallHook(addon) {
  return function (req, res, next) {
    function sendError({ code, message, ctx }) {
      addon.logger.warn(
        ctx,
    ...
    verifyJwtAndGetClaims(addon, req, res, false, true)
      .then(verifiedParams => {
        const reqHandler = requestHandler(addon, verifiedParams);
        reqHandler(req, res, next);
      })
