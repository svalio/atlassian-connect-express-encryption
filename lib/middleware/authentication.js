...
authentication.authenticate = function (addon) {
    ...
    function extractJwtFromRequest(req) {
        ...
        var authHeader = req.headers[AUTH_HEADER]; // the header name appears as lower-case
        ...
        // TODO: Remove when we deprecate the old token middleware
        if (!token) {
    ...
    return function (req, res, next) {
        ...
        if (!queryStringHash && issuer != addon.key) { // session JWT tokens that originate from the add-on don't require a qsh
        ...
        addon.settings.get('clientInfo', clientKey).then(function (settings) {
            ...
            function success(verifiedClaims) {
                ...
                requestHandler(addon)(req, res, next, {
                    clientKey: clientKey,
            ...
            function createSessionToken(verifiedClaims) {
                ...
                var token = jwt.encode({
                    ...
                    'iat': now.unix(),
                    'exp': now.add(addon.config.maxTokenAge(), 'minutes').unix(),
                    'aud': [ clientKey ]
