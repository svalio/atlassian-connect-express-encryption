    ...
    return function (req, res, next) {
        ...
        addon.settings.get('clientInfo', clientKey).then(function (settings) {
            ...
            if (!skipQshVerification && verifiedClaims.qsh) {
                var expectedHash = jwt.createQueryStringHash(jwt.fromExpressRequest(request), false, addon.config.baseUrl.href);
                var signatureHashVerified = verifiedClaims.qsh === expectedHash;
                if (!signatureHashVerified) {
                    var canonicalRequest = jwt.createCanonicalRequest(jwt.fromExpressRequest(request), false, addon.config.baseUrl.href);

                    // If that didn't verify, it might be a post/put - check the request body too
                    expectedHash = jwt.createQueryStringHash(jwt.fromExpressRequest(request), true, addon.config.baseUrl.href);
                    signatureHashVerified = verifiedClaims.qsh === expectedHash;
                    if (!signatureHashVerified) {
                        canonicalRequest = jwt.createCanonicalRequest(jwt.fromExpressRequest(request), true, addon.config.baseUrl.href);
