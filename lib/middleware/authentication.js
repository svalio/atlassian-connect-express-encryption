...
authentication.authenticate = function (addon, skipQshVerification) {
    ...
    return function (req, res, next) {
        ...
        function sendError(code, msg, ctx = {}) {
            addon.logger.warn(ctx, `Authentication verification error (${code}):  ${msg}`);
        ...
        if (!token) {
            sendError(401, 'Could not find authentication data on request', { ctx: _.omit(req.body, ['sharedSecret', 'publicKey']) });
            return;
        }
