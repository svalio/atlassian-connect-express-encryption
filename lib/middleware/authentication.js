...
function getKey(header, callback) {
  request.get(
    `https://cs-migrations--cdn.us-west-2.prod.public.atl-paas.net/${header.kid}`,
    (_err, key) => {
      if (_err) {
        return Promise.reject({
          code: 401,
          message: `Could not get public key with keyId ${header.kid}`,
          ctx: {}
        });
      }
      callback(null, key);
...
function verifyAndGetClaims(token, isSigned, secret) {
  if (!isSigned) {
    return jwt.decode(token, secret, !secret);
  }

  jwt.verify(token, getKey, { algorithm: "RS256" }, (_err, decoded) => {
    return decoded;
  });
}

async function getVerifiedClaims(addon, req, res, skipQshVerification) {
  const isSigned = addon.config.signedInstall();
  ...
  try {
    unverifiedClaims = verifyAndGetClaims(token, isSigned, ""); // decode without verification;
  } catch (e) {
    return Promise.reject({
      code: 401,
  ...
  let verifiedClaims;
  try {
    verifiedClaims = verifyAndGetClaims(token, isSigned, secret);
  } catch (error) {
    return Promise.reject({
      code: 400,
...
function verifyInstallHook(addon) {
  return function (req, res, next) {
    ...
    getVerifiedClaims(addon, req, res)
      .then(verifiedParams => {
        const reqHandler = requestHandler(addon, verifiedParams);
        reqHandler(req, res, next);
      })
