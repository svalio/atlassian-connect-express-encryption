...
async function verifyJwtAndGetClaims(
  addon,
  req,
  res,
  skipQshVerification,
  ...
  try {
    unverifiedClaims = isSignedInstallHook
      ? await decodeAsymmetricToken(token, true)
      : decodeSymmetricToken(token, "", true); // decode without verification;
  } catch (e) {
    return Promise.reject({
      code: 401,
  ...
  try {
    verifiedClaims = isSignedInstallHook
      ? await decodeAsymmetricToken(token, false)
      : decodeSymmetricToken(token, secret, false);
  } catch (error) {
    return Promise.reject({
      code: 400,
