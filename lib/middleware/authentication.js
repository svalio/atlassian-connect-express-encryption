...
authentication.authenticate = function(addon, skipQshVerification) {
    ...
    if (!tokenInQuery && !req.body) {
      addon.logger.warn(
        "Cannot find JWT token in query parameters. " +
          ...
          "Otherwise please ensure the " +
          JWT_PARAM +
          " parameter is presented in query."
      );
      return;
    }
  ...
  return function(req, res, next) {
    function sendError(code, msg, ctx = {}) {
      addon.logger.warn(
        ctx,
        `Authentication verification error (${code}):  ${msg}`
      );
    ...
    addon.settings.get("clientInfo", clientKey).then(
      function(settings) {
        function success(verifiedClaims) {
          const token = createSessionToken(verifiedClaims);
        ...
        function createSessionToken(verifiedClaims) {
          ...
          const token = jwt.encode(baseJwt, settings.sharedSecret);
        ...
        if (!settings) {
          sendError(
            401,
            "Could not find stored client data for " +
              clientKey +
        ...
        if (!secret) {
          sendError(
            401,
            "Could not find JWT sharedSecret in stored client data for " +
              clientKey
          );
          return;
        }
        ...
        if (!skipQshVerification && verifiedClaims.qsh) {
          ...
          if (!signatureHashVerified) {
            ...
            if (!signatureHashVerified) {
              ...
              addon.logger.error(
                'Auth failure: Query hash mismatch: Received: "' +
                  verifiedClaims.qsh +
                  '" but calculated "' +
                  expectedHash +
                  '". ' +
