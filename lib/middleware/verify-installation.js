...
function verifyInstallation(addon) {
    return function (req, res, next) {
        ...
        addon.settings.get('clientInfo', regInfo.clientKey).then(function (settings) {
            if (settings) {
                addon.logger.info('Found existing settings for client ' + regInfo.clientKey + '. Authenticating reinstall request');
                addon.authenticate()(req, res, next);
            } else {
                next();
            }
        });
