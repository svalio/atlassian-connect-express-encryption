  ...
  return function (req, res, next) {
    ...
    tokenHandler.verify(tokenValue, maxTokenAge,
      function(verifiedToken) {
        ...
        var ctx = res.locals;
        ctx.hostBaseUrl = verifiedToken.h;
        ctx.clientKey = verifiedToken.k;
        ctx.userId = verifiedToken.u;
        if (allowTokenRefresh) {
          ctx.token = tokenHandler.refresh(verifiedToken);
          res.setHeader(TOKEN_KEY_HEADER, ctx.token);
        }
        next();
      },
      function(error) {
