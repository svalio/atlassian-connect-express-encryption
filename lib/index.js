...
function Addon(app, opts, logger) {
  ...
  self.on('plugin_enabled', function (key, settings) {
    self.settings.set('clientInfo', settings, key).then(function(data){
      self.emit('host_settings_saved', key, data);
    });
  });

  if (self.app.get('env') === 'development' && self.config.watch()) {
    self.logger.info('Watching Connect descriptor (addon.json) for changes');
    self.watcher = fs.watch('addon.json', {persistent: false}, function (event, filename) {
      if (event === 'change') {
        self.logger.info('Re-registering due to addon.json change')
        self.register(true);
      }
    });
  }
            ...
            .then(function(hcCapabilities){
              getAccessToken(hcCapabilities.capabilities.oauth2Provider.tokenUrl, req.body.oauthId, req.body.oauthSecret)
                .then(function(tokenObj){
                  tokenObj.clientId = req.body.oauthId;
                  tokenObj.oauthSecret = req.body.oauthSecret;
                  self.emit('plugin_enabled', tokenObj.clientId, tokenObj);
                  res.send(204);
                })
  ...
  try {
    ...
    Addon.settings.get('clientInfo', clientId).then(function(d){
      ...
      try {
        ...
        req.jwt_context = request.context;
        next();
      } catch(e){
