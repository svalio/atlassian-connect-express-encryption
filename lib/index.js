...
function Addon(app, opts, logger) {
  ...
  self.schema = self.config.db();
  self.settings2 = self.schema.define('AddonSettings',{
    baseUrl:        { type: String, length: 255 },
    publicKey:      { type: String, length: 1024 },
    description:    { type: String, length: 255 },
    pluginsVersion: { type: String, length: 20 },
    clientKey:      { type: String, length: 80 },
    serverVersion:  { type: String, length: 6 },
    key:            { type: String, length: 255 },
    productType:    { type: String, length: 80 }
  });
  self.settings2.schema.isActual(function(err, actual) {
    if (!actual) {
        self.settings2.schema.autoupdate();
    }
  });
  ...
  self.on("remote_plugin_installed", function (key, settings) {
    self.settings2.create(settings);
    // self.settings.hset(settings.clientKey, settings).then(function(){
