...
proto.register = function () {
  ...
  if (hosts) {
    self.logger.info("Registering plugin...".bold);
    ...
    return Q.allResolved(hosts.map(_.bind(register, self))).then(
          ...
          if (host) {
            self.on("remote_plugin_installed", function (key, settings) {
              if (host.url === settings.baseUrl) {
                self.logger.info(("Registered with " + host.url + ".").bold);
      ...
      function () {
        self.logger.error(arguments.join(" ").red.bold)
      }
    );
  }
  return Q.resolve();
...
function register(host) {
  ...
  request.post({
    ...
    if ((err && err.code !== "ECONNREFUSED") || res && res.statusCode !== 200) {
      ...
      self.logger.error(args.join(" ").bold.red);
      dfd.resolve();
    }
...
proto.deregister = function () {
  ...
  var hosts = this.config.hosts();
  if (hosts) {
    self.logger.info("Deregistering plugin...".bold);
      ...
      function (promises) {
        promises.forEach(function (promise) {
          ...
          if (host) {
            ...
            self.logger.info(("Deregistered with host " + host.url + ".").bold);
          }
        });
      },
      function () {
