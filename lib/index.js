...
const verifySignedInstallation = require("./middleware/verify-installation");
...
class Addon extends EventEmitter {
  constructor(app, opts, logger, callback) {
    ...
    self.signedInstall = self.descriptor.signedInstall || false;
  ...
  _configure(callback) {
    ...
    const descriptorUrl = baseUrlInternal.clone().segment(DESCRIPTOR_FILENAME);
  ...
  verifyInstallHook() {
    return authentication.verifyInstallHook(this);
  }
