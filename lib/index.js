...
proto._configure = function (callback) {
    ...
    fs.readFile(DESCRIPTOR_FILENAME, 'utf8', function(error, descriptorText) {
        ...
        var baseUrl = new URI(rawDescriptorBaseUrl.replace('{{localBaseUrl}}', self.config.localBaseUrl()));
        self.config.baseUrl = urls.parse(baseUrl.toString());

        var descriptorUrl = baseUrl.clone().segment(DESCRIPTOR_FILENAME);
        ...
        if (modules && modules.webhooks) {
            ...
            webhooks.forEach(function (webhook) {
                ...
                var webhookUrl = baseUrl.clone().segment(webhook.url);
                self.app.post(
                    // mount path
                    webhookUrl.path(),
                    // auth middleware
        ...
        var installUrl = baseUrl.clone().segment('/installed');
