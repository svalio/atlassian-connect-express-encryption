...
proto.register = function () {
  ...
  if (hosts) {
    console.log("Registering plugin...".bold);
    ...
    return Q.allResolved(hosts.map(_.bind(register, self))).then(
          ...
          if (host) {
            self.on("remote_plugin_installed", function (key, settings) {
              if (host.url === settings.baseUrl) {
                console.log(("Registered with " + host.url + ".").bold);
      ...
      function () {
        console.error(arguments.join(" ").red.bold)
      }
    );
  }
  return Q.resolve();
...
function register(host) {
  ...
  request.post({
    ...
    if ((err && err.code !== "ECONNREFUSED") || res && res.statusCode !== 200) {
      ...
      console.error(args.join(" ").bold.red);
      dfd.resolve();
    }
...
proto.deregister = function () {
  ...
  var hosts = this.config.hosts();
  if (hosts) {
    console.log("Deregistering plugin...".bold);
      ...
      function (promises) {
        promises.forEach(function (promise) {
          ...
          if (host) {
            ...
            console.log(("Deregistered with host " + host.url + ".").bold);
          }
        });
      },
      function () {
