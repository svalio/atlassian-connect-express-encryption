...
exports.validateDescriptor = function() {
  ...
  return new RSVP.Promise(function(resolve, reject) {
    ...
    getGlobalProductSchema(productName).then(
      function(schema) {
        var results = [];
        validator.validateDescriptor(descriptor, schema, function(
          errors,
          warnings
        ) {
          if (errors) {
    ...
    request.get(
      ...
      function(error, response, body) {
        ...
        if (statusCode < 200 || statusCode > 299) {
          self.logger.error(
            "Could not download schema from " +
              url +
              " : " +
              (error || statusCode)
          );
...
function register(forceRegistration, hostRegUrl) {
  ...
  var descriptorUrl = new URI(self.config.localBaseUrl())
    .segment("atlassian-connect.json")
    .toString();
  ...
  return new RSVP.Promise(function(resolve, reject) {
    request.get(reqObject, function(err, res, body) {
      function doReg() {
        ...
        reqObject.jar = false;
        request.post(reqObject, function(err, res) {
...
function waitForRegistrationResult(hostRegUrl, body) {
  ...
  var callForRegistrationResult = function(lastBody) {
    ...
    return new RSVP.Promise(function(resolve, reject) {
      ...
      setTimeout(function() {
        request.get(reqObject, function(err, res) {
          if (err || (res && (res.statusCode < 200 || res.statusCode > 299))) {
            return reject([err, res]);
          }
...
function deregisterUpm(self, hostRegUrl) {
  return new RSVP.Promise(function(resolve, reject) {
    var reqObject = getUrlRequestObject(
      hostRegUrl,
      "/rest/plugins/1.0/" + self.key + "-key"
    );
    reqObject.jar = false;
    request.del(reqObject, function(err, res) {
      if (err || (res && (res.statusCode < 200 || res.statusCode > 299))) {
        return reject([err, res]);
      }
      resolve();
