...
function checkUpmRegistered(hostRegUrl, pluginKey) {
    ...
    return new RSVP.Promise(function (resolve, reject) {
        request.get({
            uri: stripCredentials(hostRegUrl) + '/rest/plugins/1.0/',
            jar: false,
            auth: {
                user: username,
                pass: password,
    ...
    return new RSVP.Promise(function (resolve, reject) {
        request.head({
            ...
            function doReg() {
                ...
                request.post({
                    uri: stripCredentials(hostRegUrl) + '/rest/plugins/1.0/?token=' + upmToken,
...
function waitForRegistrationResult(hostRegUrl, body) {
    ...
    var url = new URI(hostRegUrl).path(body.links.self);
    var username = url.username();
    var password = url.password();
    url.username('');
    url.password('');
