...
function requireNgrok() {
    return requireOptional.requireOptional('ngrok');
}
...
function createTunnel(addon) {
    ...
    return requireNgrok()
        .then(function (ngrok) {
            return RSVP.denodeify(ngrok.connect)(addon.config.port());
        })
        .then(function (url) {
            var ltu = new URI(url);
...
exports.getGlobalProductSchema = function() {
    var self = this;
    ...
    return new RSVP.Promise(function(resolve, reject) {
        ...
        self.logger.info('Validating descriptor with schema ' + url);
...
function requireGlobalSchema() {
    return requireOptional.requireOptional('getGlobalProductSchema');
}
    ...
    return new RSVP.Promise(function (resolve, reject) {
        ...
        requireGlobalSchema()
            .then(function(schema) {
                var results = [];
                validator.validateDescriptor(self.descriptor, schema, function (errors, warnings) {
                    if (errors) {
