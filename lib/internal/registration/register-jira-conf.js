...
function createTunnel(addon) {
    ...
    return requireNgrok()
        .then(function (ngrok) {
            const ngrokPromise = ngrok.connect({ proto: 'http', addr: addon.config.port() });
            if (!ngrokPromise) {
                return RSVP.reject('You must update ngrok to >= 3.0');
            }
            return ngrokPromise;
        })
        .then(function (url) {
        ...
        .catch(function (err) {
            ...
            if (err.code === 'MODULE_NOT_FOUND') {
                addon.logger.error('Make sure that ngrok is installed: npm install --save-dev ngrok');
            }
...
exports.validateDescriptor = function() {
    ...
    return new RSVP.Promise(function (resolve, reject) {
        self.logger.info('Trying to validate the app descriptor. The app will still continue to run even on validation errors and warnings. This is just to inform you of potential mistakes in descriptor');

        var product = self.config.product();
        var productName = product.isJIRA ? 'jira' : 'confluence';

        var descriptor = self.descriptor;

        getGlobalProductSchema(productName)
            .then(function(schema) {
                var results = [];
                validator.validateDescriptor(descriptor, schema, function (errors, warnings) {
                    if (errors) {
                        results.push(getValidationFailCause('error', 'Validation errors', errors));
                    }

                    if (warnings) {
                        results.push(getValidationFailCause('warning', 'Unexpected attributes: The descriptor is valid, but double-check for typos and elements in the wrong place', warnings));
                    }

                    if (results.length > 0) {
                        self.logger.info('Error validating app descriptor: [' + JSON.stringify(results, null, 2) + '] . Please check and resolve the problem/s. ' +
...
function getGlobalProductSchema(productName) {
    var self = this;
    return new RSVP.Promise(function(resolve, reject) {
        if(!_.isEmpty(productSchema)) return resolve(productSchema);

        var docsSchemaUrlFormat = process.env.SCHEMA_HOST || 'https://developer.atlassian.com/static/connect/docs/latest/schema/%s-global-schema.json';
        var url = util.format(docsSchemaUrlFormat, productName);
        ...
        request.get({
            ...
            function (error, response, body) {
                var statusCode = (!!error || !response) ? 500 : response.statusCode;
                if (statusCode < 200 || statusCode > 299) {
                    self.logger.error('Could not download schema from ' + url + ' : ' + (error || statusCode));
                    return reject(error);
                } else {
                    productSchema = body;
...
function getValidationFailCause(type, message, cause) {
    ...
    cause.forEach(result => validationResults.push(_.pick(result, ['module', 'value', 'validValues', 'description'])));
...
function registrationError(action, hostUrl, err, res) {
    var hostBaseUrl = stripCredentials(hostUrl);
    var args = ['Failed to ' + action + ' with host ' + hostBaseUrl];
