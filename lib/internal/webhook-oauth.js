...
function webhookOAuth(plugin, basePath) {
  ...
  return function (req, res, next) {
    ...
    if (!installed || path !== installedUrl) {
      ...
      if (!baseUrl) {
        return sendError("No baseUrl provide in registration info.");
      }
      ...
      // next verify with the provided publicKey; this could be spoofed, but we will verify the key itplugin
      ...
      plugin.authenticate(publicKey)(req, res, function () {
        // in order to protect against the aforementioned spoofing, we need to next make a request back
