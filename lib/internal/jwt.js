...
jwt.createQueryStringHash = function createQueryStringHash(req) {
    return crypto.createHash(algorithmMap.HS256).update(this._createCanonicalRequest(req)).digest('hex');
};
    ...
    if (!_.isEmpty(query)) {
        ...
        _.each(_.keys(query).sort(), function (key) {
            var param = query[key],
                paramValue = '';
            if (_.isArray(param)) {
                paramValue = _.map(param.sort(), function (v) { return encodeURIComponent(v); }).join(',');
            } else {
