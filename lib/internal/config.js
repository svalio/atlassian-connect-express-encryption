...
module.exports = function (config, mode, overrides) {
  ...
  return _.extend({}, wrap(globalValues), wrap(modeValues), {
    ...
    signedInstall() {
      // `signedInstall` is `true` by default. Explicitly configure signedInstall to false to opt-out.
      return env["AC_SIGNED_INSTALL"] !== false && globalValues["signedInstall"] !== false;
    },
