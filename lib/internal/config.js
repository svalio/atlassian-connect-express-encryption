...
module.exports = function (mode) {
  ...
  function tunneledBaseUrl(){
    var client = lt_client.connect({
      host: 'http://localtunnel.me',
      port: get(modeValues, "port", "PORT")
    });
  }

  function getLocalBaseUrl(){
    if (!env['AP3_LOCAL_BASE_URL'] && !modeValues['localBaseUrl']) {
      var hostnames = _.map(get(modeValues, "hosts"), function(host){
        return url.parse(host).hostname;
      });
      if(_.contains(hostnames, 'localhost')){
