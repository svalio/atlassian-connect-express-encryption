    ...
    ['get', 'post', 'put', 'del', 'head', 'patch'].forEach(function (method) {
        ...
        hostClient[method] = function (options, callback) {
            return addon.settings.get('clientInfo', clientKey).then(function (clientSettings) {
                ...
                // AC-1274: 'form', if passed to request.js, implies 'application/x-www-form-urlencoded',
                // but the form handling below assumes 'multipart/form-data'. For compatibility reasons
                // we continue to support 'form', but remove it from the options passed to request.js
                var multipartFormData = options.form;
                delete options.form;
