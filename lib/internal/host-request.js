...
HostClient.prototype.getUserBearerToken = function (clientSettings) {
    var cachedToken = false;


    if (cachedToken && moment(cachedToken.expiresAt).isBefore(moment())) {
        return RSVP.Promise.resolve(cachedToken);
    }
    ...
    var opts = {
        ...
        scopes: this.addon.descriptor.scopes
    }

    // get new token
    return oauth2.getAccessToken(opts);
};
        ...
        return this.addon.settings.get('clientInfo', this.clientKey).then(function (clientSettings) {
            ...
            var augmentHeaders = function (headers, relativeUri) {
                ...
                if (!self.userKey) {
                    ...
                    var bearerToken = self.getUserBearerToken(clientSettings);
