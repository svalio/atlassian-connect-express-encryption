...
function checkNotNullAndNotEmpty(thing, name) {
    if (!thing) {
...
module.exports = function (addon, context, clientKey) {
    ...
    checkNotNullAndNotEmpty(addon, 'addon');
    checkNotNullAndNotEmpty(addon.settings, 'addon.settings');
        ...
        hostClient[method] = function (options, callback) {
            return addon.settings.get('clientInfo', clientKey).then(function (clientSettings) {
                ...
                var augmentHeaders = function (headers, relativeUri) {
                    var indexOfQuestionMark = relativeUri.indexOf('?');
                    var path = indexOfQuestionMark > 0 ? relativeUri.substr(0, indexOfQuestionMark) : relativeUri;
                    var queryString = indexOfQuestionMark > 0 ? relativeUri.substr(indexOfQuestionMark + 1) : '';
                    var queryPairs = queryString ? queryString.split('&') : [];
                    var query = {};
                    for (var i in queryPairs) {
                        var nameAndValue = queryPairs[i].split('=');
