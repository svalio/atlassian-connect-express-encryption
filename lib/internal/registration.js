        ...
        hostInfo.get({
            ...
            timeout: 5000
                ...
                function (info) {
                    ...
                    function fail(args) {
                        self.logger.warn(registrationError('register', clientKey, args[0], args[1]));
                ...
                function (err) {
                    self.logger.error("Could not contact host:", stripCredentials(hostRegUrl));
                    reject(err);
                }
        );
    });
...
function waitForRegistertrationResult(hostRegUrl, body) {
    ...
    var url = new URI(hostRegUrl).path(body.links.self).toString();
...
function deregister(hostRegUrl) {
    ...
    return new RSVP.Promise(function (resolve, reject) {
        ...
        function fail(args) {
            self.logger.warn(registrationError('deregister', clientKey, args[0], args[1]));
            resolve();
        }
