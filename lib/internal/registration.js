        ...
        createTunnel(self).then(
                function () {
                    if (hostRegUrls && hostRegUrls.length > 0) {
                        ...
                        var gracefulRereg = /graceful-rereg/.test(process.env.AC_OPTS) && !isReregistration;
                        RSVP.all(hostRegUrls.map(_.bind(register, self, gracefulRereg))).then(
                                function () {
...
function register(gracefulRereg, hostRegUrl) {
    var self = this;
    ...
    return new RSVP.Promise(function (resolve, reject) {
        hostInfo.get(hostRegUrl).then(
                function (info) {
                    ...
                    registerUpm(hostRegUrl, descriptorUrl, self.descriptor.key, gracefulRereg).then(done, fail);
                },
                function () {
...
function registerUpm(hostRegUrl, descriptorUrl, pluginKey, gracefulRereg) {
    return new RSVP.Promise(function (resolve, reject) {
        request.head({
            uri: hostRegUrl + '/rest/plugins/1.0/',
            jar: false
