...
exports.deregister = function () {
  ...
  var promise;
  if (!/no-dereg/.test(process.env.AC_OPTS) && hostRegUrls.length > 0) {
...
function register(hostRegUrl) {
  ...
  if (self.config.type() === 'hipchat') {
    return registerHipChat(self.config, hostRegUrl, descriptorUrl).then(
      function(clientKey) {
        self.logger.info('Registered with host ' + clientKey + ' at ' + hostRegUrl);
        self._registrations[hostRegUrl] = clientKey;
      }, function(args) {
        self.logger.error(registrationError('register', hostRegUrl, args[0], args[1]));
      }
    );
  } else {
  ...
  var promise = new RSVP.Promise(function(resolve,reject) {
    request.head({
      ...
      request.post({
        ...
        if (err || (res && res.statusCode !== 202)) return reject([err, res]);
        resolve();
      });
    });
...
function registerHipChat(config, hostRegUrl, descriptorUrl) {
  var groupId = config.groupId();
  if (!groupId) {
    throw Exception("No groupId defined in config");
  }

  var room = config.room();
  return new RSVP.Promise(function(resolve,reject) {
    request.post({
                   uri: hostRegUrl + '/v2/marketplace/internal?_internal=' + groupId,
                   headers: {'content-type': 'application/json'},
                   body: JSON.stringify({capabilities_url: descriptorUrl})},
                 function (err, res) {
                    if (err || (res && res.statusCode !== 201)) return reject([err, res]);
                    var pluginKey = JSON.parse(res.body).key;
                    request.post({
                                 uri: hostRegUrl + '/v2/addon?_internal=' + groupId,
                                 headers: {'content-type': 'application/json'},
                                 body: JSON.stringify({key: pluginKey, room: room})},
                                 function (err, res) {
                                    if (err || (res && res.statusCode !== 201)) return reject([err, res]);
                                    resolve(JSON.parse(res.body).id);
                                });
    });
  });
}
...
function deregister(hostRegUrl) {
  ...
  var promise = new RSVP.Promise(function(resolve,reject) {
    ...
    if (clientKey && self.config.type() !== 'hipchat') {
...
function registrationError(action, clientKey, err, res) {
  ...
  if (res && res.body && !/^<[^h]*html[^>]*>/i.test(res.body)) args.push(res.body);
  return args.join('\n');
}
