...
function waitForRegistertrationResult(hostRegUrl, body) {
    var waitTime = body.pingAfter || 200;
    return new RSVP.Promise(function(resolve, reject) {
        setTimeout(function(){
            var url = new URI(hostRegUrl).segment(body.links.self);
            ...
            request.get({
                ...
                if (results.status && results.status.done) {
                    ...
                    waitForRegistertrationResult(hostRegUrl, results).then(resolve, reject);
                }
            });
        }, waitTime);
    });
