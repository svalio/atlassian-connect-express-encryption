...
exports.deregister = function () {
  var self = this;
  if (self._registrations.length > 0) {
    self.logger.info("Deregistering plugin...");
    return Q.allResolved(self._registrations.map(_.bind(deregister, self)));
  }
  return Q.resolve();
};
...
function register(hostRegUrl) {
  ...
  request.post({
    ...
    if ((err && err.code !== "ECONNREFUSED") || res && res.statusCode !== 200) {
      var args = ["Failed to register with " + bareHost + " (" + res.statusCode + ")."];
      if (err) args.push("\n" + err);
      if (res.body) args.push("\n" + res.body);
      ...
      self.on("remote_plugin_installed", function (key, settings) {
        if (probablyEqualUrls(bareHost, settings.baseUrl)) {
          ...
          self._registrations.push(hostRegUrl);
        }
      });
...
function deregister(hostRegUrl) {
  ...
  request.del({
    ...
    if ((err && err.code !== "ECONNREFUSED") || res && res.statusCode !== 204) {
      var args = ["Failed to deregister with " + bareHost + " (" + res.statusCode + ")."];
      if (err) args.push("\n" + err);
      if (res.body) args.push("\n" + res.body);
      self.logger.error.apply(self.logger, args);
      dfd.resolve();
      ...
      if (hostRegUrl.key) {
        function resolve() { dfd.resolve(hostRegUrl); }
        self.settings.hdel(hostRegUrl.key).then(resolve, resolve);
      }
      else {
