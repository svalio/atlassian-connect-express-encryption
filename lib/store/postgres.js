...
module.exports = function (logger, config) {
  ...
  function reject(err) {
    logger.error("Postgres initialization error; using memory store.\n" + err);
    proxyDfd.resolve(require("./index").create("memory", logger));
  }
  ...
  client.exec("CREATE EXTENSION IF NOT EXISTS hstore;").then(
    function () {
      ...
      client.exec("CREATE TABLE IF NOT EXISTS " + table + " (id serial, key varchar(64) PRIMARY KEY, hash hstore);").then(
        function () {
          // all ready
          logger.info("Postgres settings store initialized.");
