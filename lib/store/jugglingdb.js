...
// returns null if nothing was found, otherwise a single value if 'resultIsArray' is false, otherwise an array of values
proto._get = function(where, resultIsArray) {
    var self = this;
    var promise = new RSVP.Promise(function(resolve,reject) {
        self.promise.then(function(){
            self._AddonSettings.all({where:where}, function(err, arry){
                ...
                if (resultIsArray) return resolve(_rowsToValues(arry));
                resolve(arry[0].val);
            });
        });
    });
...
proto.getAllClientInfos = function () {
  return this._get({key:'clientInfo'}, true);
};
...
proto.get = function (key, clientKey) {
  return this._get({key: key, clientKey: clientKey}, false);
};
