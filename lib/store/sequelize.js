...
class SequelizeAdapter {
  ...
  async get(key, clientKey) {
    ...
    const Cryptor = require("../../../../server/helpers/Cryptor");
    const val = result ? getAsObject(result.get("val")) : null;
    if (val) {
      const encryptedSharedSecret = val.encryptedSharedSecret;
      if (encryptedSharedSecret) {
        val.sharedSecret = await new Cryptor().decrypt(encryptedSharedSecret);
        return val;
      }
    }
